...
commands_list = [
    "encender luces de cabina", "apagar luces de cabina", "encender luces de lectura", "apagar luces de lectura",
    "abrir puerta delantera", "abrir puerta trasera", "cerrar puerta delantera", "cerrar puerta trasera"
]
...
class SpeechRecognizer:
    ...
    async def process_audio(self):
        ...
        if self.recognizer.AcceptWaveform(processed_data):
            ...
            if recognized_text:
                await CommandProcessor.process_command(recognized_text, self)

class CommandProcessor:
    @staticmethod
    async def process_command(recognized_text, recognizer):
        ...
        if keyword_found:
            ...
            if matched_command and (matched_command != recognizer.last_command_time or current_time - recognizer.last_command_time > 5):
                ...
                await CommandHandler.queue_command(matched_command)
                latency = time.time() - start_time
                logging.info(f"Latencia del comando '{matched_command}': {latency:.2f} segundos")
                await CommandHandler.schedule_training(matched_command, time.localtime().tm_hour)
            else:
...
class CommandHandler:
    ...
    def get_best_match(cls, command):
        ...
        confidence_threshold = cls.confidence_threshold_night if 22 <= current_hour or current_hour <= 6 else cls.confidence_threshold_day
    ...
    @classmethod
    async def queue_command(cls, command):
        await cls.command_queue.put(command)
        await asyncio.get_running_loop().run_in_executor(cls.executor, cls.execute_command, command)
    ...
    @classmethod
    async def schedule_training(cls, command, hour):
